...
import { Calendar, Armchair, CheckCircle2, AlertCircle, BookmarkCheck, Loader2, Info, Trash2 } from 'lucide-react';
...
const BookingGrid = () => {
    ...
    const handleCancel = async () => {
        ...
        try {
            setLoading(true);
            // Assuming your backend supports DELETE on /api/bookings/cancel/{id}
            // If it uses a different path, adjust accordingly
            await axios.delete(`http://localhost:8080/api/bookings/cancel/${myBooking.id}`, {
                withCredentials: true
            });
            ...
            const errorMsg = error.response?.data || 'Cancellation failed.';
            setMessage({ type: 'error', text: errorMsg });
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6 pb-24 animate-in fade-in duration-700">
            ...
            {/* User's Current Booking Card with CANCEL BUTTON */}
            {myBooking && (
                <div className="bg-emerald-600 p-8 rounded-[2.5rem] text-white flex items-center justify-between shadow-2xl shadow-emerald-200/50 animate-in zoom-in-95 duration-500">
                    ...
                    <button
                        ...
                        className="bg-rose-500 hover:bg-rose-400 text-white px-6 py-3 rounded-2xl font-black flex items-center gap-2 transition-all active:scale-95 shadow-lg border border-rose-400/50"
                    >
                        <Trash2 size={18} />
                        Cancel Booking
                    </button>
